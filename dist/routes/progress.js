...
progress.get("/", (c) => {
    ...
    return c.html(_jsx(ProgressPage, { user: user, entries: userProgress.entries, cycle: userProgress.cycle, target: userProgress.target, progressPercent: userProgress.progressPercent, currentPosition: userProgress.currentPosition, ramadanYear: ramadanYear, success: success, error: error }));
});
progress.post("/", async (c) => {
    ...
    db.prepare(`INSERT INTO progress_entries (user_id, surah_number, last_ayah, completed)
     VALUES (?, ?, ?, ?)
    ...
    if (lastAyah !== previousAyah) {
        db.prepare("INSERT INTO progress_log (user_id, surah_number, ayah_from, ayah_to) VALUES (?, ?, ?, ?)").run(user.id, surahNumber, previousAyah, lastAyah);
    }
    const msg = completed
